...
function CategoriesSectionWatchlist({ title, id, fetcher }) {
  return (
...
function CategoriesContent({ fetcher }) {
  ...
  return (
    <ul className="flex gap-4 w-full scrollbar-hide scroll-on-hover">
      {data.map((post) => (
        <Link
          href={
            post.media_type?.length
              ? getWatchUrl(post.id, post.media_type, post.poster_path)
        ...
        >
          <Image
            ...
            quality={30}
          />
          <div className="flex justify-center w-[210px]">
...
export default CategoriesSectionWatchlist;
