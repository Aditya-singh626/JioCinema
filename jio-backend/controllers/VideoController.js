...
const getAllVideos = async (req, res) => {
  try {
    ...
    // filtering and mapping the files to get only .mp4 files and their names
    const mp4Files = files.filter(
      (file) => path.extname(file).toLowerCase() === ".mp4",
    );
    const videoList = mp4Files.map((file) => ({
      //path.parse(file).name → Gets the filename without extension (used as id).
      id: path.parse(file).name,
      //name: file → Full filename  without _ and extension .
      name: path.parse(file).name.replace(/_/g, " "),
    }));
    // To log the fetched video list for debugging purposes
